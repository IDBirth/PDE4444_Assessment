...
# ── Optimized-export manifest lookup ────────────────────────────────────────

_OPTIMIZED_MANIFEST = REPO_ROOT / "top_models" / "manifest.csv"


def lookup_optimized_manifest(path: Path) -> dict | None:
    """If `path` is an optimized export (rank{NN}_{name}.{ext}) under
    top_models/model_optimized/, return its manifest row. Otherwise None."""
    if not _OPTIMIZED_MANIFEST.exists():
        return None
    stem = path.stem
    with _OPTIMIZED_MANIFEST.open(newline="") as fh:
        for row in csv.DictReader(fh):
            expected = f"rank{int(row['rank']):02d}_{row['name']}"
            if expected == stem:
                return row
    return None
...
def detect_model_type(path: Path) -> str:
    ...
    # Optimized exports (.torchscript / .onnx / .engine): use manifest as source
    # of truth. The file itself carries no reliable family hint — a YOLO
    # torchscript loaded via YOLO() defaults to task='detect' without a hint.
    if suffix in {".torchscript", ".onnx", ".engine"}:
        entry = lookup_optimized_manifest(path)
        if entry is None:
            raise ValueError(
                f"Optimized file {path.name} not found in manifest; "
                f"expected stem 'rank{{NN}}_{{name}}' matching top_models/manifest.csv."
            )
        family = entry["model_type"].replace("_finetuned", "")
        if family == "yolo":
            return "yolo"
        if suffix == ".torchscript":
            return f"torchscript_{family}"
        raise ValueError(
            f"{suffix} export for {family} is not supported by the demo; "
            f"use the .torchscript variant (more portable) or the .pt checkpoint."
        )

    # Raw .pt checkpoint — try YOLO first, then state-dict inspection.
    try:
        m = YOLO(str(path))
...
class DemoModel:
    def __init__(
        ...
        if self.model_type == "yolo":
            # .torchscript / .onnx / .engine don't carry the task — tell YOLO.
            if model_path.suffix.lower() in {".torchscript", ".onnx", ".engine"}:
                self._yolo = YOLO(str(model_path), task="classify")
            else:
        ...
        elif self.model_type.startswith("torchscript_"):
            # Self-contained scripted module — no architecture rebuild needed.
            entry = lookup_optimized_manifest(model_path) or {}
            img_size = int(entry.get("img_size") or 224)
            family   = self.model_type.split("_", 1)[1]  # "mobilenet" | "mlp"
            scripted = torch.jit.load(str(model_path), map_location=DEVICE).eval()
            self.class_names = ["non_defect", "defect"]
            if family == "mobilenet":
                self._mobilenet = scripted
                self._transform = build_mobilenet_transform(img_size)
                self._mobilenet_img_size = img_size
                self.model_type = "mobilenet"   # re-use existing predict branch
            else:
                self._mlp          = scripted
                self._mlp_img_size = img_size
